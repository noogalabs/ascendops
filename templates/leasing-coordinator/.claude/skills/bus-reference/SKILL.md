...
### Browser and connector automation
Browser automation and external data connectors are not bundled in operator templates. Pull the connector skills you need (browser automation, your property-management data connector, endpoint tooling) from your MCP connectors during onboarding (see the agentic PM MCP step), then wire them into `.mcp.json`.
