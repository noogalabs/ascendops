...
## Incident-Driven Skill Audits

Skill audits fire on real failure moments, not just the calendar (locked 2026-07-01, orchestration-upgrade Tip 1 P2). When any of these four incidents occurs, dispatch a targeted skill-optimizer audit at the agent/skill involved:

1. **David correction** — David corrects, redoes, or negatively flags an agent's skill-driven output (including yours)
2. **Guardrail trip** — a `guardrail_triggered` event appears in any agent's event log
3. **Rejected approval** — an approval is resolved `rejected`
4. **Skill-driven task failure** — a task fails or errors while an agent was following a skill

Identify the agent and the specific SKILL.md the agent was following when the incident happened (from the incident itself, event meta, the agent's daily memory, or their heartbeat at that time). If no specific skill is identifiable, skip — this rule targets skill-driven failures only.

Dedup guard — max one dispatch per agent+skill per 7 days:

```bash
GUARD="$CTX_FRAMEWORK_ROOT/orgs/$CTX_ORG/agents/$CTX_AGENT_NAME/state/incident-audit-dispatches.log"
mkdir -p "$(dirname "$GUARD")"
CUTOFF=$(date -u -v-7d +%Y-%m-%d 2>/dev/null || date -u -d '7 days ago' +%Y-%m-%d)
awk -v c="$CUTOFF" '$1 >= c' "$GUARD" 2>/dev/null | grep -q "agent=<agent> skill=<skill>" && echo "DEDUP — already dispatched this week, skip"
```

If not deduped, dispatch to an agent (audit owner — holds the skill-optimizer skill and transcript access) and record it:

```bash
cortextos bus send-message an agent high 'INCIDENT SKILL AUDIT — run skill-optimizer now, outside the calendar cron. Treat this as SKILL_OPTIMIZER_TARGET=<skill> (the override hook in skill-optimizer SKILL.md). Audited agent: <agent>. Skill: your org internal docs<agent>/.claude/skills/<skill>/SKILL.md. Transcript: newest JSONL in ~/.claude/projects/-Users-davidhunter-cortextos-orgs-ascendops-agents-<agent>/ covering the incident window. Incident: <one line — what happened and when>. Write analysis.md, diff.patch and history.json to skill-improvement/<skill>/ as usual, record the audited agent name in analysis.md and in the history run entry ("agent": "<agent>"), and reply to me with score + top gap. The diff gets triaged in my weekly-review apply loop.'
echo "$(date -u +%Y-%m-%d) agent=<agent> skill=<skill> trigger=<david-correction|guardrail|rejected-approval|task-failure>" >> "$GUARD"
cortextos bus log-event action task_dispatched info --meta '{"to":"an agent","task":"incident skill audit <agent>/<skill>","trigger":"<trigger>"}'
```

Triggers (1) and (3) fire the moment you see them (David correction in conversation; rejected-approval inbox message). Triggers (2) and (4) are also swept daily: morning review (overnight window) and evening review Phase 2D (today window).
