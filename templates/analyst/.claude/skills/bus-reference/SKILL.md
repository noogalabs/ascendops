...
### auto-commit
Acquire the state-side single-writer lease, then stage only the invoking agent's allowlisted state. The command refuses a nonempty index, reports the actual post-operation index, returns the exact release token, and never pushes. Follow the local-version-control skill through commit-then-release or unstage-then-release.

```bash
cortextos bus auto-commit [--dry-run]
cortextos bus auto-commit-lease-status
cortextos bus auto-commit-assert-held <token>
cortextos bus auto-commit-release <token>
```
...
| Snapshot allowlisted agent state  | `auto-commit`             |
