...
if [[ "$HEARTBEAT_NOOP" != "true" ]]; then
  ...
  # POSITIVE LANE GATE — require ONE before any reminder:
  #   (a) OWNED: this agent created/requested the task and still owns its lane;
  #   (b) BLOCKING: it directly blocks a goal-linked task this agent owns now;
  #   (c) GRANTED: OPERATING_MODEL.md gives this agent org-wide queue ownership.
  ...
  # For an out-of-lane task: do not contact David; escalate internally to the
  # orchestrator over the bus. Project membership is discovery, not authority.
...
1. Alert the user via direct Telegram API using WebFetch
...
**Never claim a status you haven't verified.** Crons are daemon-managed and survive restarts — confirm with `ascendops bus list-crons` before saying "crons running."
