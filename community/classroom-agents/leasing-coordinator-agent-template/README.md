...
- `applicant-screening-digest` (`0 8 * * 1-5`), Run the applicant-screening skill in digest mode: score new applications against the written rubric and dra....
- `renewal-window-am` (`0 8 * * 1-5`), Run renewals-coordinator in morning mode: detect leases entering the renewal window, score risk off payment....
- `renewal-window-pm` (`0 17 * * 1-5`), Run renewals-coordinator in evening mode: surface changed renewal flags and deadline pressure only.
- `lease-abstraction-intake` (`0 9 * * 1-5`), Run lease-abstraction on any newly received leases: extract terms into structured data and flag missing, am....
- `fair-housing-presend-sweep` (`30 8 * * *`), Run fair-housing-guard over any pending applicant- or resident-facing drafts and screening criteria.
