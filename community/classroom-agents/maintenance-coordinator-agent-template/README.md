...
- `intake-sweep` (``), Run the intake-triage skill on any new inbound maintenance requests: categorize, rank severity, decide tena....
- `sla-watch` (``), Run vendor-coordination SLA review: flag silent vendors that have not confirmed a window, response/completi....
- `open-wo-digest` (`0 8 * * 1-5`), Run an open work-order digest: list every open ticket with severity, vendor status, SLA state, and what is ....
- `make-ready-review` (`0 9 * * 1-5`), Run make-ready-scheduling for active turns: refresh the trade sequence, recompute the critical path, and fl....
